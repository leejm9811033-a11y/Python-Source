...
# render_template_string : 문자열로 작성된 jinja 템플릿을 렌더링 해 HTML로 반환
...
# 유지하는 것 처럼 할 수 있다. 서버가 설정 -> 브라우저가 저장 -> 다음 요청부터 브라우저가 자동으로 함께 전송
...
@app.get("/read_cookie")
def read_cookie():
    # 브라우저가 요청에 실어 보낸 모든 쿠키 중에서내 서버가 만든 name 쿠키를 꺼냄
