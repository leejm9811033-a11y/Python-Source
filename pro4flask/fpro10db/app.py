...
import pymysql
import os
from flask import get_flashed_messages  # 저장해둔 메시지를 꺼내는 함수
# flash("에러~")    => 메시지를 세션에 잠시 저장 후 get_flashed_messages() 하면 메세지를 읽음. 휘발성임(단발성). 그래서 끝나면 사라짐. 
...
@app.get("/show/")
def show_list():
    conn = get_conn()  # db와 연결 하고 작업해.
    try:
        with conn.cursor() as cur:
            cur.execute("select code, sang, su, dan from sangdata order by code")
            rows = cur.fetchall()

        messages = list(get_flashed_messages()) # 메시지를 잠시 받아놔.
    ...
    # except pymysql.err.IntegrityError as e:
        ...
        conn.close()   # db와 연결 끊어. 이래야 db한테 좋음. 철칙!!!
